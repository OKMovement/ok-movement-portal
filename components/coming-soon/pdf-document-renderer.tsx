...
function PdfPageItem({
  ...
  useEffect(() => {
    if (shouldRender || !pageWrapperRef.current || !rootRef.current) {
      return;
    }
  ...
  return (
    <div
      ...
      style={{
        minHeight: pageWidth > 0 ? `${Math.round(pageWidth * pageRatio)}px` : undefined,
      }}
    >
...
export default function PdfDocumentRenderer({
  ...
  useEffect(() => {
    ...
    const updateMetrics = () => {
      ...
      // Keep mobile pages sharp when users pinch-zoom, while capping desktop cost.
      setDevicePixelRatio(
        viewportWidth < 640
          ? Math.min(currentDevicePixelRatio, 2.5)
          : Math.min(currentDevicePixelRatio, 1.75),
      );
    };
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div
        ref={scrollRootRef}
        className="min-h-0 flex-1 overflow-y-auto bg-neutral-900/80 px-0 py-0 sm:px-2 sm:py-3"
      >
